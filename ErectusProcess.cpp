...
{
    // Allocate with RW only - no execute permission (stealth)
...
bool ErectusProcess::ReprotectWrite(const std::uintptr_t addr, const void* data, const size_t size)
{
    if (!addr)
        return false;
    ...
    // Step 1: Flip RX → RW
    DWORD oldProtect;
    if (!VirtualProtectEx(handle, reinterpret_cast<void*>(addr), size, PAGE_READWRITE, &oldProtect))
        return false;

    // Step 2: Write updated data
    const bool writeOk = Wpm(addr, data, size);

    // Step 3: Flip back RW → RX
    DWORD dummy;
    VirtualProtectEx(handle, reinterpret_cast<void*>(addr), size, PAGE_EXECUTE_READ, &dummy);
